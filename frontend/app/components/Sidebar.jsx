...
export function Sidebar({ currentUser, currentPage, onNavigate, isOpen = true, onClose }) {
  ...
  const userNavItems = [
    ...
    { icon: Tv, label: '채널 둘러보기', page: 'channels' },
    { icon: CreditCard, label: '내 구독', page: 'my-subscriptions' },
    { icon: Heart, label: '구매한 콘텐츠', page: 'my-purchases' },
    { icon: MessageSquare, label: '내 리뷰', page: 'my-reviews' },
  ];

  // Creator Navigation
  const creatorNavItems = [
    { icon: LayoutDashboard, label: '대시보드', page: 'creator-dashboard' },
    { icon: Tv, label: '채널 관리', page: 'creator-channel' },
    { icon: Video, label: '콘텐츠 관리', page: 'creator-content' },
    { icon: DollarSign, label: '정산 관리', page: 'creator-settlement' },
  ];

  // Admin Navigation
  const adminNavItems = [
    { icon: Users, label: '판매자 신청', page: 'admin-applications' },
    { icon: CreditCard, label: '결제 내역', page: 'admin-payments' },
    { icon: FileText, label: '정산 관리', page: 'admin-settlements' },
  ];
