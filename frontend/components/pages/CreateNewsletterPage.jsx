...
import { Mail, ArrowLeft, Save, X } from 'lucide-react';
import { createNewsletter } from '@/app/lib/api';
...
export function CreateNewsletterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      setError('');
      await createNewsletter(title.trim(), content.trim());
      alert('뉴스레터가 저장되었습니다.');
      router.push('/admin/newsletters');
    } catch (err) {
      setError(err.message || '뉴스레터 저장 중 오류가 발생했습니다.');
      console.error('뉴스레터 생성 오류:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto pb-12">
      <div className="mb-6">
        ...
        <p className="text-gray-600">뉴스레터를 작성하고 저장하세요</p>
      </div>
