...
import { createContent, getMyChannel } from '@/app/lib/api';

export function CreatorContentNewPage({ onNavigate }) {
  ...
  const [formData, setFormData] = React.useState({
    ...
    mediaUrl: '',
    price: 0,
    publishedAt: null, // null이면 임시저장, 값이 있으면 예약 발행
  });
  ...
  React.useEffect(() => {
    async function loadChannel() {
      try {
        ...
        const errorMessage = err?.response?.data?.message || err?.message || '채널 정보를 불러올 수 없습니다.';
  ...
  // 발행 옵션: 'immediate' (즉시 발행, 기본값), 'draft' (임시저장), 'scheduled' (예약 발행)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!channelId) {
        throw new Error('채널 ID를 찾을 수 없습니다. 채널 정보를 확인해주세요.');
      }
      ...
      const successMessage = 
        publishOption === 'immediate' ? '콘텐츠가 발행되었습니다.' :
        publishOption === 'scheduled' ? '콘텐츠가 예약 발행되었습니다.' :
        '콘텐츠가 임시저장되었습니다.';
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">새 콘텐츠 등록</h1>
          <p className="text-gray-600">새로운 콘텐츠를 만들고 공유하세요</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm p-6 space-y-6">
        ...
        <div className="space-y-4 border-t pt-6">
          ...
          <div className="space-y-3">
            ...
            <div
              onClick={() => {
                // 이미 선택된 옵션을 다시 클릭하면 즉시 발행으로 되돌림
